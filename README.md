...
- `TerminalUI`：零依赖渲染彩色横幅、思考、完整命令、命令输出、执行状态和最终回答；非 TTY 自动关闭颜色。
...
交互模式默认启用适合 Demo 录制的终端界面，依次展示模型等待状态、`reasoning_content`、工具调用、完整命令、有界命令输出和最终回答。脚本调用或需要稳定纯文本时可使用：
...
39 项单元测试覆盖 terminal、终端 UI、配置、上下文、模型输出解析、agent 循环、规划检查点和流式协议。真实 DeepSeek V4 Flash 除完成三个修复类任务外，还从六个全空目录执行了同一句 2048 高层任务；完整日志和独立行为验证暴露了范围膨胀、重复验证、“只看退出码”的假阳性、文本化工具协议和上下文成本问题，并推动了推理预算、规划节奏、证据检查、协议恢复和分层上下文的通用优化。详细过程见 [`EVALUATION.md`](EVALUATION.md)。
